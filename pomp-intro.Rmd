...
x %>% ggplot(aes(x=time,y=y,group=sim,color=(sim=="data")))+
...
traj %>% ggplot(aes(x=time,y=N))+geom_line()
```
...
")
parus %>%
  pomp(rprocess=discrete.time.sim(step.fun=stochStep,delta.t=1),
...
sim <- simulate(parus,params=c(N.0=1,e.0=0,r=12,sigma=0.5),as.data.frame=TRUE,states=TRUE)
...
```{r parus-add-meas}
parus %>%
  pomp(rmeasure=rmeas,dmeasure=dmeas,statenames=c("N"),paramnames=c("phi")) -> parus
```
...
simulate(parus,nsim=3,as.data.frame=TRUE,include.data=TRUE) %>%
