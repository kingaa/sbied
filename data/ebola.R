...
mdat <- read.csv(text=WHO.situation.report.Oct.1,stringsAsFactors=FALSE)
mdat <- melt(mdat,id="week",variable.name="country",value.name="cases")
mutate(mdat,deaths=NA) -> mdat
...
ebolaModel <- function (country=c("Guinea", "SierraLeone", "Liberia", "WestAfrica"),
  ...
  if (is.null(data)) {
    if (ctry=="WestAfrica") {
      dat <- ddply(mdat,~week,summarize,
                   cases=sum(cases,na.rm=TRUE),
                   deaths=sum(deaths,na.rm=TRUE))
    } else {
      dat <- subset(mdat,country==ctry,select=-country)
    }
  } else {
    dat <- data
  }
