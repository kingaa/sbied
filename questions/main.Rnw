...
\myquestion. Effective sample size (ESS) is one of the main tools for diagnosing the success of a particle filter. It is shown if you plot an object of class \texttt{pfilterd\_pomp} which is created by applying \texttt{pfilter} to a \texttt{pomp} object. Suppose one or more time points have low ESS (say, less than 10) even when using a fairly large number of particles (say, $10^4$). What is the proper interpretation?
...
\item A likelihood slice can be used to compute confidence intervals.
\item A ppor man's profile must lie above the true profile.
\end{enumerate}
...
\myquestion. The iterated filtering convergence diagnostics plot shown above come from a \href{https://ionides.github.io/531w21/final_project/project06/blinded.html}{\textcolor{blue}{student project}}. What is the best interpretation?
\begin{enumerate}[label=(\Alph*)]
...
\myquestion. The iterated filtering convergence diagnostics plot shown above come from a \href{https://ionides.github.io/531w21/final_project/project15/blinded.html}{\textcolor{blue}{student project}}, calculated using $10^3$ particles. What is the best interpretation?
\begin{enumerate}[label=(\Alph*)]
