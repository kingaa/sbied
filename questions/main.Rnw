...
\textcolor{red}{[Should the correct s.e. be 1.4?]}
\begin{enumerate}[label=(\Alph*)]
...
\myquestion. Effective sample size (ESS) is one of the main tools for diagnosing the success of a particle filter.
If you plot an object of class \texttt{pfilterd\_pomp} (created by applying \texttt{pfilter} to a \texttt{pomp} object), the ESS is displayed.
