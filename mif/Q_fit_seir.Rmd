...
This consistently obtains log likelihoods around -105, similar to those found with the SIR model:
```{r}
...
```{r global_search_design}
  ...
  fixed_params <- coef(measSEIR,c("mu_IR","k","N"))
  mf1 <- mifs_local[[1]]
```
...
The maximum log likelihood discovered is `r round(max(results$loglik),1)`, which confirms that the mode being investigated earlier by the local search is also the global mode. 

```{r scatter_plot}
pairs(~loglik+Beta+eta+rho+mu_EI,data=filter(results,loglik>max(loglik)-20))
```

This shows quite a different pattern from the corresponding figure for the SIR model in Section 5 of Lesson 4.
After adding a latent class, higher values of the reporting rate and lower values of the initial susceptible fraction, $\eta$, are preferred.
This regime matches our epidemiological expectations for endemic measles in the pre-vaccine era.
