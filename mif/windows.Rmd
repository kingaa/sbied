...
```{r knitr-opts,include=FALSE,purl=FALSE,child="../setup.Rmd",eval=FALSE}
```
...
As an illustration, we will consider two code snippets.
First, we load some packages, set up a parallel environment, and build the SIR model we have seen in the Lessons.
...
Now compare the following two code snippets.
...
The two differences between the codes are:
