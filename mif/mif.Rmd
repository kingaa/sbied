...
```{r load_bbs}
bsflu_data <- read.table("bsflu_data.csv")
```
...
For the code, we represent the states ($S$, $I$, $R_1$, $R_2$), the observations ($B$, $C$) and the parameters ($\beta$, $\mu_I$, $\rho$, $\mu_{R_1}$, $\mu_{R_2}$) as follows:
...
It can also be helpful to build the pomp object with some set of preliminary values for the other parameters, to test simulation and filtering:

```{r regular_params}
bsflu_regular_params <- c(beta=0.004, mu_I=0.7, rho=0.9)
```
...
bsflu <- pomp(
     ...
     times="day",
     t0=0,
     params=c(bsflu_regular_params,bsflu_fixed_params),
     rprocess=euler.sim(
...
```{r run_level}
run_level <- 3
bsflu_Np <-          c(100,2e4, 6e4)
bsflu_Nmif <-        c(10, 100, 300)
bsflu_Neval <-       c(2,  10,  10)
bsflu_Nglobal <-     c(10, 10,  100)
bsflu_Nlocal <-     c(10, 10,  20)
```
...
```{r bsflu_params}
bsflu_params <- data.matrix(read.table("bsflu_params.csv",row.names=NULL,header=TRUE))
bsflu_mle <- bsflu_params[which.max(bsflu_params[,"logLik"]),][bsflu_paramnames]
```

It is convenient to do some parallelization to speed up the computations. Most machines are multi-core nowadays, and using this computational capacity involves only (i) the following three lines of code to let R know you plan to use multiple processors; (ii) using the parallel for loop provided by 'foreach'.
Since I am working on a double-threaded 12-cpu machine, I set
...
t_pf <- system.time(
  pf <- foreach(i=1:cores,.packages='pomp') %dopar% try(
    pfilter(bsflu,params=bsflu_mle,Np=bsflu_Np[run_level],seed=297221+i)
  )
)
...
t_local <- system.time({
  mifs_local <- foreach(i=1:bsflu_Nlocal[run_level],.packages='pomp', .combine=c) %dopar%  {
     mif2(
        bsflu,
        start=bsflu_mle,
        seed=743275+i,
        Np=bsflu_Np[run_level],
        Nmif=bsflu_Nmif[run_level],
...
t_local_eval <- system.time({
  liks_local <- foreach(i=1:bsflu_Nlocal[run_level],.packages='pomp',.combine=rbind) %dopar% {
    set.seed(87932+i)
    evals <- replicate(bsflu_Neval[run_level], logLik(pfilter(bsflu,params=coef(mifs_local[[i]]),Np=bsflu_Np[run_level])))
...
t_global <- system.time({
  mifs_global <- foreach(i=1:bsflu_Nglobal[run_level],.packages='pomp', .combine=c) %dopar%  mif2(
     mifs_local[[1]],
...
t_global_eval <- system.time({
  liks_global <- foreach(i=1:bsflu_Nglobal[run_level],.packages='pomp',.combine=rbind) %dopar% {
    set.seed(87932+i)
    evals <- replicate(bsflu_Neval[run_level], logLik(pfilter(bsflu,params=coef(mifs_global[[i]]),Np=bsflu_Np[run_level])))
    logmeanexp(evals, se=TRUE)
  }
})
...
if(run_level>1) write.table(rbind(results_local,results_global),
   file="bsflu_params.csv",append=TRUE,col.names=FALSE,row.names=FALSE)
```
...
## Technical exercise: Checking the model source code.
