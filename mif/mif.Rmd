...
```{r local_search_plot,purl=FALSE}
mifs_local %>%
  traces() %>%
  melt() %>%
...
read_csv("measles_params.csv") %>%
  ...
  bind_rows(guesses) %>%
  mutate(type=if_else(is.na(loglik),"guess","result")) -> all
...
```
```{r purl=FALSE,include=FALSE}
n_beta <- getDoParWorkers()
t_beta <- attr(results,"system.time")
```
```{r}
results %>%
  write_csv(path="measles_params.csv",append=TRUE)
