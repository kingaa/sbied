...
We fix `cooling.fraction.50=0.5`, so that 50 `mif2` iterations result in a 50% decrease in the perturbation magnitudes.
...
This investigation took  `r round(t_local["elapsed"]/60,1)` minutes for the maximization and `r round(t_local["elapsed"]/60,1)` minutes for the likelihood evaluation. These repeated stochastic maximizations can also show us the geometry of the likelihood surface in a neighborhood of this point estimate:
...
re-running a command on an object created by the same command preserves the algorithmic arguments.
