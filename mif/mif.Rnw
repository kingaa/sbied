...
\begin{frame}{Introduction}
  ...
  \item This tutorial follows on from the \htmladdnormallink{topic of carrying out particle filtering (also known as sequential Monte Carlo) via \code{pfilter} in \package{pomp}}{../pfilter/pfilter.html}. 
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile,allowframebreaks]{Estimating the likelihood}
  ...
  This local investigation took \Sexpr{round(attr(mifs_local,"system.time")["elapsed"],0)}~sec for the maximization and \Sexpr{round(t_local["elapsed"],0)}~sec for the likelihood evaluation.
...
\begin{frame}{Extra Exercise: Choosing the algorithmic settings for IF2}
  Have a look at \htmladdnormallink{\textcolor{blue}{our advice on tuning IF2}}{./if2_settings.html}.
\end{frame}
