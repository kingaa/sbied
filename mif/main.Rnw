...
\begin{frame}[fragile]
  ...
  The following loads the data and the stochastic SIR model we constructed there.
...
\begin{frame}[fragile,allowframebreaks=0.8]{Parallel computing}
  ...
  \link{https://kingaa.github.io/sbied/pfilter/}{Lesson 3} discusses how to accomplish this using \package{foreach}.
...
\begin{frame}[fragile]{Windows issues}
  ...
  \link{./windows.html}{Have a look at this document to learn about the workaround.}
...
\begin{frame}{\myexercise. Fitting all parameters}
  ...
  Attempt to estimate all the parameters simultaneously.
