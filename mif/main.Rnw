...
\begin{frame}{Introduction}
  ...
  \item This tutorial follows on from the \link{../pfilter/slides.pdf}{topic of particle filtering} (also known as sequential Monte Carlo) via \code{pfilter} in \package{pomp}. 
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile,allowframebreaks]{Implementation in \package{pomp}}
...
\begin{frame}[fragile,allowframebreaks=0.95]{Testing the codes: filtering}
  ...
  In this case, the ESS is very small at several points.
...
\begin{frame}[fragile,allowframebreaks]
  ...
  $\beta\in (5,80)$, $\rho\in (0.2,0.9)$, $\eta\in (0,0.4)$.
...
\begin{frame}[fragile,allowframebreaks]{Another global search}
  ...
  See the \link{https://raw.githubusercontent.com/kingaa/sbied/master/mif/main.R}{\Rlanguage code for this lesson} to see exactly how this is done.
...
\begin{frame}{\myexercise. Fitting the SEIR model}
...
\begin{frame}{\myexercise. Modify the measurement model}
  The binomial measurement model used here assumes that the reporting rate is fixed through time and that all infection reports are independent and equally likely.
  Commonly, it is advisable to consider measurement models with \myemph{overdispersion}, i.e., more variability than the binomial (or Poisson) allows.
  The negative binomial distribution, for instance, allows for overdispersed mean-variance relationships.
  Similarly, the beta-binomial distribution adds overdispersion to the binomial.
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \link{https://raw.githubusercontent.com/kingaa/sbied/master/mif/main.R}{\Rlanguage code for this lesson}
