...
Question: Why do we use multiplicative noise, rather than additive noise such as 
...
bake(file="box_search_global.rds",{
  ...
  {
    mf <- mif2(mf1,start=c(unlist(guess),fixed_params),Nmif=NMIF)
...
library(plyr)
all <- ldply( list(guess=guesses, result=subset(results_global, loglik > max(loglik)-50) ), .id="type")
...
bake(file="box_search_global_sigma.rds",{
  ...
  ) %dopar% 
  {
    ...
    ll <- replicate(N_LIK_REPS,logLik(pfilter(mf,Np=NP_MIF)))
    ll <- logmeanexp(ll,se=TRUE)
    c(coef(mf),loglik=ll[1],loglik=ll[2])
  }
...
* We see that dynamic overdispersion, by itself, gives substantially less improvement in model fit.
...
* measurement overdispersion alone is apparently sufficient to improve the fit over $H_{\bullet\bullet}$. It also greatly reduces the filtering error.
...
* Let's look at the fitted MLE models under these different constraints
```{r MLEs}
