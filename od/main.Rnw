...
\item  This investigation provides a relatively simple example to better understand overdispersed Markov chain models that are essential for large populations such as the [city-level measles case study](../measles/measles.html).
...
\begin{frame}{background on mean-variance relationships and overdispersion}
...
\begin{frame}[fragile]{Adding overdispersion to the basic SIR model}

\begin{itemize}
\item Recall the case report data on the 1948 measles outbreak in Consett, UK.
\end{itemize}
...
\item The profile for $\rho$ shows very little pattern for $\sigma$, which could be because the amount of information about $\sigma$ is small in the context of this model (i.e., the profile has low curvature -- it is close to flat).
