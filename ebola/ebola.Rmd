...
This document contains the data and codes needed to fully reproduce all the results in ["Avoidable errors in the modelling of outbreaks of emerging pathogens, with special reference to Ebola"](http://dx.doi.org/10.1098/rspb.2015.0347) [@King2015].  These codes are also accessible as appendix C in this paper and via [datadryad.org](http://datadryad.org/resource/doi:10.5061/dryad.r5f30).    
It is dedicated to the public domain as per the [Creative Commons CC0 public domain dedication](http://creativecommons.org/publicdomain/zero/1.0/).  Please mention its origin.  ![CC0](../graphics/cc0.png)  
It was produced using `pomp` version `r packageVersion("pomp")`.

```{r include=FALSE,purl=FALSE}
library(knitr)

prefix <- "avoidable_errors"
opts_chunk$set(
  progress=TRUE,
...
The following installs these packages, if they are not already installed.
...
Many of the codes below are designed to be run on a cluster using OpenMPI.
It will be assumed that an MPI hostfile, `hosts`, exists, which specifes the hosts to be used.
An example `hosts` file has contents
```
localhost slots=1 max-slots=1
node1 slots=0 max-slots=10
node2 slots=0 max-slots=10
node3 slots=0 max-slots=10
node4 slots=0 max-slots=10
```

Here, `node1`, `node2`, `node3`, and `node4` are the compute nodes in this small cluster.
Each has at least 10 cores.

## Model implementation

The following is the contents of the file `ebola-model.R`, which will be loaded by the each of the codes below.
It defines a function `ebolaModel` that constructs a `pomp` object holding the Ebola data and the model codes.

Contents of the file `ebola-model.R`:
```{r ebola,eval=FALSE}
require(pomp)
require(plyr)
require(reshape2)
...
The following is the contents of a file, `simstudy.R`, which runs the simulation study computations.
Note that it assumes that file `ebola-model.R` (contents displayed above) is present in the working directory.
On a cluster with `R` and `OpenMPI` installed, in a directory containing `simstudy.R`, `ebola-model.R`, and the hostfile `hosts`, execute the computations with a command such as:
```
mpirun -hostfile hosts -np 41 Rscript --vanilla simstudy.R
```

Contents of file `simstudy.R`:
```{r simstudy,eval=FALSE}
require(pomp)
require(plyr)
require(reshape2)
require(magrittr)
...
The following are the contents of the file `profiles.R`, which uses trajectory matching and iterated filtering to estimate model parameters for all four countries and both types of data.
Again, this is designed to be run on an MPI cluster.
In a directory with the files `hosts` and `ebola-model.R` (see above), execute these computations via a command like
```
...
The file `diagnostics.R`, whose contents are shown below, computes the diagnostics displayed in the paper.
These computations are not very heavy, but can be accelerated using `doMC` if run on a multi-core workstation.
In a directory containing `ebola-profiles.rds` (see above) and `ebola-model.R`, execute these codes with a command like
```
