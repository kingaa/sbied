...
\myexercise. A widely used, exact simulation method for continuous time Markov chains is  \href{https://en.wikipedia.org/wiki/Gillespie_algorithm}{Gillspie's algorithm}.
...
\item  We follow method 3 above, modeling the number, $\dlta{N_{SI}}$, moving from S to I over interval $\dlta{t}$ as
...
\href{./exercises.html#basic-exercise-explore-the-sir-model}{\mylink{Worked solution to the Exercise}}
...
\href{./exercises.html#basic-exercise-the-seir-model}{\mylink{Worked solution to the Exercise}}
