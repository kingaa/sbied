...
\frametitle{A basic compartment model: The SIR model}
...
\input{../graphics/sir_diagram.tex}
\vspace{5mm}
...
\frametitle{Recovering compartment processes from counting proceeses  \rule[-2mm]{0mm}{3mm}}
...
\frametitle{The ordinary differential equation (ODE) interpretation of the SIR model \rule[-2mm]{0mm}{3mm}}
...
\frametitle{The simple continuous-time Markov chain interpretation of the SIR model  \rule[-2mm]{0mm}{3mm}}
...
\myexercise. What is the link between little $o$ notation and the derivative?
Explain why 
...
\href{https://kingaa.github.io/sbied/stochsim/exercises.html#little-o-notation}{\mylink{Worked solution to the Exercise}}
...
\begin{frame}{Euler's method for ordinary differential equations (ODEs)}
...
\item He supposed this ODE has some true solution $x(t)$ which could not be worked out analytically. He wanted to approximate $x(t)$ numerically.
...
\myexercise. A widely used, exact simulation method for continuous time Markov chains is  \href{https://en.wikipedia.org/wiki/Gillespie_algorithm}{\mylink{Gillspie's algorithm}}.
...
\href{https://kingaa.github.io/sbied/stochsim/exercises.html#euler-versus-gillespie}{\mylink{Worked solution to the Exercise}}

\vspace{3mm}

Numerically, Gillespie's algorithm is often approximated using so-called
\href{https://en.wikipedia.org/wiki/Tau-leaping}{\mylink{tau-leaping}}
...
\frametitle{Compartment models in \package{pomp}: The Consett measles outbreak \rule[-2mm]{0mm}{3mm}}
...
\item As an example that we can probe in some depth, let's look at outbreak of measles that occurred in the small town of Consett in England in 1948.

% <!--- population 38820, 737 births in the year. --->

\item We download the data and examine them:
...
\frametitle{Implementing the SIR model in \package{pomp}}
...
<<add-meas-modelR>>=
measSIR <- pomp(measSIR,rmeasure=sir_rmeas,dmeasure=sir_dmeas)
@
...
\item In the data, it looks like there were a total of $\Sexpr{sum(meas$reports)}$ infections.
  ...
  $\eta=\frac{S_0}{N}\approx\Sexpr{signif(2*sum(meas$reports)/38000,2)}$.
...
<<sir_sim1_plot,echo=F,eval=T,out.width="8cm">>=
<<sir_sim1_code>>
@
...
\myexercise. Explore the SIR model. Fiddle with the parameters to see if you can't find a model for which the data are a more plausible realization.

\vspace{3mm}

\href{https://kingaa.github.io/sbied/stochsim/exercises.html#basic-exercise-explore-the-sir-model}{\mylink{Worked solution to the Exercise}}
...
\myexercise. The SEIR model. Below is a diagram of the so-called SEIR model.
This differs from the SIR model in that infected individuals must pass a period of latency before becoming infectious.

<<seir-diagram,echo=FALSE,purl=FALSE,fig.height=3/4,fig.width=15/2>>=
library(grid)
...
\href{https://kingaa.github.io/sbied/stochsim/exercises.html#basic-exercise-the-seir-model}{\mylink{Worked solution to the Exercise}}
