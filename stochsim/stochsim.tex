...
\frametitle{Compartmental models in \package{pomp}: The Consett measles outbreak}
...
\begin{alltt}
\hlkwd{read_csv}\hlstd{(}\hlstr{"https://kingaa.github.io/sbied/stochsim/Measles_Consett_1948.csv"}\hlstd{)} \hlopt{%>%}
  \hlkwd{select}\hlstd{(week,}\hlkwc{reports}\hlstd{=cases)} \hlkwb{->} \hlstd{meas}
\hlkwd{head}\hlstd{(}\hlkwd{as.data.frame}\hlstd{(meas),}\hlnum{5}\hlstd{)}
\end{alltt}
\begin{verbatim}
##   week reports
## 1    1       0
...
\hlstd{measSIR} \hlopt{%>%}
  \hlkwd{pomp}\hlstd{(}
    \hlkwc{rprocess}\hlstd{=}\hlkwd{euler}\hlstd{(sir_step,}\hlkwc{delta.t}\hlstd{=}\hlnum{1}\hlopt{/}\hlnum{7}\hlstd{),}
    \hlkwc{rinit}\hlstd{=sir_init,}\hlkwc{accumvars}\hlstd{=}\hlstr{"H"}
  \hlstd{)} \hlkwb{->} \hlstd{measSIR}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{dmeas} \hlkwb{<-} \hlkwa{function} \hlstd{(}\hlkwc{reports}\hlstd{,} \hlkwc{H}\hlstd{,} \hlkwc{rho}\hlstd{,} \hlkwc{log}\hlstd{,} \hlkwc{...}\hlstd{) \{}
 \hlkwd{dbinom}\hlstd{(}\hlkwc{x}\hlstd{=reports,} \hlkwc{size}\hlstd{=H,} \hlkwc{prob}\hlstd{=rho,} \hlkwc{log}\hlstd{=log)}
\hlstd{\}}

\hlstd{rmeas} \hlkwb{<-} \hlkwa{function} \hlstd{(}\hlkwc{H}\hlstd{,} \hlkwc{rho}\hlstd{,} \hlkwc{...}\hlstd{) \{}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{measSIR} \hlopt{%>%} \hlkwd{pomp}\hlstd{(}\hlkwc{rmeasure}\hlstd{=rmeas,}\hlkwc{dmeasure}\hlstd{=dmeas)} \hlkwb{->} \hlstd{measSIR}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\item For example, a C snippets encoding the rprocess for an \code{sir} model is as follows.
...
\item Note that, when using C snippets, one has to tell \code{pomp} which of the variables referenced in the C snippets are state variables and which are parameters.
...
\myquestion. Explore the SIR model. Fiddle with the parameters to see if you can't find a model for which the data are a more plausible realization.

\href{./exercises.html#basic-exercise-explore-the-sir-model}{Worked solution to the Exercise}
...
\item Produced with R version 4.0.1 and \package{pomp} version 3.0.1.0.
