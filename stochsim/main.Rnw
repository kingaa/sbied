...
<<doParallel,echo=FALSE,cache=FALSE>>=
library(doParallel)
library(doRNG)
registerDoParallel()
...
\begin{frame}[allowframebreaks=0.8]{General notation for compartment models}
  \bi
\item To develop a systematic notation, it turns out to be convenient to keep track of the flows between compartments as well as the number of individuals in each compartment. Let
  $$N_{SI}(t)$$
...
\item Now, we'll model the data as a binomial process,
  \begin{equation*}
    \mathrm{reports}_t \sim \dist{Binomial}{H(t),\rho}.
  \end{equation*}
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \link{https://raw.githubusercontent.com/kingaa/sbied/master/stochsim/main.R}{\Rlanguage codes for this lesson}
\end{frame}
