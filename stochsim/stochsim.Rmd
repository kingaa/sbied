...
1. We develop deterministic and stochastic interpretations of a compartment model.
...
We will do this using notation which generalizes to more complex systems [[@breto09]](http://dept.stat.lsa.umich.edu/~ionides/pubs/breto09.pdf).

```{r sir-diagram,echo=FALSE,purl=FALSE,fig.height=7/9}
library(grid)
vp <- viewport(width=unit(32/6,"inches"),height=unit(2/3,"inches"))
pushViewport(vp)
grid.rect(x=c(1/4,2/4,3/4),y=1/2,width=1/8,height=1,just=c(0.5,0.5),gp=gpar(fill="white",lwd=2))
grid.text(x=c(1/4,2/4,3/4),y=1/2,label=c(expression(S),expression(I),expression(R)),
          gp=gpar(fontsize=24))
# grid.text(x=unit(c(6/16,10/16),"npc"),y=unit(1/2,"npc")+unit(c(10,-10),"point"),
#           label=c(expression(mu[SI]),expression(mu[IR])),gp=gpar(fontsize=12))
...
- Let $S$, $I$, and $R$ represent, respectively, the number of susceptible boys, the number of infected (and infectious) boys, and the number of recovered and/or removed boys. 
...
- The rates may be either constant or varying. In particular, for a simple SIR model, the recovery rate $\mu_{IR}$ is a constant but the infection rate has the time-varying form $$\mu_{SI}(t)=\beta \, I(t),$$ with $\beta$ being the _contact rate_. 
  In the simplest case, when the population is closed, we set 
...
- The Markov property lets us specify a model by the transition probabilities on small intervals (together with the initial conditions). 
...
- [Euler](https://en.wikipedia.org/wiki/Leonhard_Euler) took the following approach to numeric solution of an ODE:
    + He wanted investigate an ODE $$\frac{dx}{dt} = h(x,t)$$
...
- In some physical and engineering situations, a system follows an ODE model closely. 
  For example, Newton's laws provide a very good approximation to the motions of celestial bodies. 
- In many biological situations, ODE models only become close mathematical approximations to reality at reasonably large scale. 
...
- If our model fitting, model investigation, and final conclusions are all based on our numerical solution  $\tilde x(t)$ (i.e., we are sticking entirely to simulation-based methods) then we are most immediately concerned with how well $\tilde x(t)$ describes the system of interest.  
...
- Here are three possibilities:
    ...
    $$\tilde N_{SI}\big( \,(k+1)\delta\,\big)= \tilde N_{SI}\big(k\delta\big) + \dist{Poisson}{\tilde \mu_{SI}(t)\,\tilde S(t)\,\delta},$$ where $\dist{Poisson}{\mu}$ is a Poisson random variable with mean $\mu$ and $$\tilde\mu_{SI}(t)=\beta\,\tilde I(t).$$
    2. Binomial increments with linear probability:
    $$\tilde N_{SI}\big((k+1)\delta\big)= \tilde N_{SI}\big(k\delta\big) + \dist{Binomial}{\tilde{S}(t),\tilde\mu_{SI}(t)\,\delta},$$ where $\dist{Binomial}{n,p}$ is a binomial random variable with mean $np$ and variance $np(1-p)$.
    3. $\tilde{N}_{SI}\big((k+1)\delta\big)=\tilde{N}_{SI}\big(k\delta\big) + \dist{Binomial}{\tilde{S}(t),1-e^{-\tilde{\mu}_{SI}(t)\,\delta}}$.  
...
#### Exercise: Compartment models via stochastic differential equations (SDEs)
...
Write down a stochastic Euler method for an SDE representation of the SIR model. 
...
#### Exercise: Euler's method vs Gillspie's algorithm
...
- Numerically, Gillespie's algorithm is often approximated using so-called [tau-leaping](https://en.wikipedia.org/wiki/Tau-leaping) methods. 
...
[Download a script containing the **R** commands below.](http://raw.githubusercontent.com/kingaa/sbied/master/stochsim/stochsim.R)
...
```{r seir-diagram,echo=FALSE,purl=FALSE,fig.height=7/9}
library(grid)
vp <- viewport(width=unit(20/3,"inches"),height=unit(2/3,"inches"))
pushViewport(vp)
fs <- 24
...
```{r sirr-diagram,echo=FALSE,purl=FALSE,fig.height=7/9}
library(grid)
vp <- viewport(width=unit(20/3,"inches"),height=unit(2/3,"inches"))
pushViewport(vp)
fs <- 24
grid.rect(x=c(1/5,2/5,3/5,4/5),y=1/2,width=1/10,height=1,just=c(0.5,0.5),gp=gpar(fill="white",lwd=2))
grid.text(x=c(1/5,2/5,3/5,4/5),y=1/2,label=c(expression(S),expression(I),expression(R[1]),expression(R[2])),gp=gpar(fontface=3,fontsize=fs))
grid.lines(x=c(5/20,7/20),y=1/2,arrow=arrow(length=unit(0.1,"npc")),gp=gpar(lwd=2))
grid.lines(x=c(9/20,11/20),y=1/2,arrow=arrow(length=unit(0.1,"npc")),gp=gpar(lwd=2))
grid.lines(x=c(13/20,15/20),y=1/2,arrow=arrow(length=unit(0.1,"npc")),gp=gpar(lwd=2))
popViewport()
