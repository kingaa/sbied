...
\item Iterated filtering for POMP models was extended to PanelPOMPs by \citet{Breto2019}. 
...
\item A \code{panelPomp} object for the above contact data and model is provided by \code{contacts} in \package{panelPomp}.
...
\item Can we do better, using the independence of units? It turns out we can \citep{Breto2019}.
...
\item The panel iterated filtering (PIF) algorithm of \citet{Breto2019} applies the IF2 algorithm to a POMP model constructed by concanenating the collection of panels.
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \item
    Produced with \package{R} version \Sexpr{getRversion()}, \package{pomp} version \Sexpr{packageVersion("pomp")}, and \package{panelPomp} version \Sexpr{packageVersion("panelPomp")}.
