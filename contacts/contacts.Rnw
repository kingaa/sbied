...
<<parallel,echo=FALSE,cache=FALSE>>=
# DEBUG <- TRUE
DEBUG <- FALSE
...
\item Plotted is a sample of 15 time series from \href{https://kingaa.github.io/sbied/contacts/contacts.csv}{\mylink{contacts.csv}}.

\ei

<<data_code, echo=T,eval=F>>=
...
\section{Interpreting a fitted model}
...
\parbox{6cm}{
<<plot_dynamics,out.width="6cm",echo=F>>=
...
\hspace{8mm}
\parbox{4.5cm}{
Fig 4 of  \citet{romero-severson15}. The median of 500 simulations are shown as lines and the $75^{th}$ and $25^{th}$ quantiles are shown as gray envelopes.
...
\section{PanelPOMP models and \package{panelPomp}}
...
\item The implementation of the above model equations in \code{contacts} can be found in the \href{https://github.com/cbreto/panelPomp/blob/master/inst/examples/pancon.R}{\mylink{\code{panelPomp} source code on github}}.
...
\myexercise. Suppose a PanelPOMP model has all its parameters unit-specific. Is there anything useful to be gained from the PanelPOMP structure, or is it preferable to analyze the data as a collection of POMP models?

\vspace{3mm}

\href{https://kingaa.github.io/sbied/contacts/Q-unit-specific.html}{\mylink{Worked solution}}
...
\myexercise. Methods for panelPomps.

\bi
\item How would you find the \package{panelPomp} package methods available for working with a \code{panelPomp} object?  
\ei

\vspace{3mm}

\href{https://kingaa.github.io/sbied/contacts/Q-panelPomp-methods.html}{\mylink{Worked solution}}


\end{frame}

\section{The likelihood for PanelPOMPs}
...
\myexercise. Consider what happens when we \code{pfilter} a \code{panelPomp}.
...
\href{https://kingaa.github.io/sbied/contacts/Q-pfilter-for-panelPomp.html}{\mylink{Worked solution}}
...
<<t1-timing_start,echo=F,cache=F>>=
timing <- !file.exists("results/pfilter1.rda")
tic <- Sys.time()
@

<<pfilter1-code,eval=F,echo=F>>=
...
\item This took \Sexpr{round(t1,1)} minutes using \Sexpr{cores} cores.
...
\frametitle{\code{logmeanexp} versus \code{panel_logmeanexp}}
...
\item The improvement via \code{panel_logmeanexp} is small in this case, since the number of observation times is small.
...
\myexercise.  The difference between \code{panel_logmeanexp} and \code{logmeanexp}
...
\item The \code{panel_logmeanexp} averages separately for each unit before combining.
...
\href{https://kingaa.github.io/sbied/contacts/Q-panel-logmeanexp.html}{\mylink{Worked solution}}
...
<<t2-timing-start,echo=F,cache=F>>=
timing <- !file.exists("results/mif1.rda")
tic <- Sys.time()
@

<<mif1-code,eval=F,echo=T>>=
...
\item This is a relatively quick search, taking \Sexpr{round(as.numeric(t2),1)} minutes.
...
<<t3-timing-start,echo=F,cache=F>>=
timing <- !file.exists("results/mif1-lik-eval.rda")
tic <- Sys.time()
@

<<mif1-lik-eval-code,eval=F,echo=T>>=
...
\bi
\item This took \Sexpr{round(as.numeric(t3),1)} minutes.
\ei
