...
\begin{frame}[fragile]
\frametitle{Chapter \CHAPTER. Case study: Panel data on dynamic variation in sexual contact rates \rule[-2.5mm]{0mm}{3mm}}
...
\begin{frame}[fragile]
\frametitle{Introduction to panel data}
...
\item \package{pomp} methods were extended to PanelPOMP models by \citet{breto19}. 
...
<<timing_pfilter1,echo=F,cache=F>>=
...
\frametitle{Combining Monte Carlo likelihood evaluations for PanelPOMPs}
...
\item Previous versions were presented at SISMID by ELI and AAK in 2015--2019.
