...
\item Introducing the **pomp** R package.
\end{itemize}
\end{frame}
...
\item To familiarize students with the **pomp** package.
...
\item POMP models can include all the features desired by @Bjornstad2001.
...
\includegraphics[width=500px]{pomp_schematic1.png}
...
\begin{itemize}
\item In the following diagram, arrows show dependence among model variables:
...
\item Moreover, the measurable random variable, $Y_n$, depends only on the state at that time:
