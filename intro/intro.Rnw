...
\title{Lesson 1:\\Introduction to Simulation-based Inference\\for Epidemiological Dynamics}
...
\begin{frame}{Objectives for this lesson}
  ...
  \item<+-> To introduce the \package{pomp} \Rlanguage package.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item<+-> To familiarize students with the \package{pomp} package.
...
\begin{frame}[allowframebreaks=0.8]{Questions and answers}
  \begin{enumerate}
  \item \link{https://doi.org/10.1126/scitranslmed.aaj1748}{How to explain the resurgence of pertussis in countries with sustained high vaccine coverage?}
  \item \link{http://dx.doi.org/10.1038/nature07084}{What roles are played by asymptomatic infection and waning immunity in cholera epidemics?}
  \item \link{http://dx.doi.org/10.1098/rsif.2009.0151}{What explains the seasonality of measles?}
  \item \link{http://dx.doi.org/10.1126/science.aat6777}{Can serotype-specific immunity explain the strain dynamics of human enteroviruses?}
  \item \link{http://dx.doi.org/10.1371/journal.pone.0072086}{Do subclinical infections of pertussis play an important epidemiological role?}
  \item \link{http://dx.doi.org/10.1093/aje/kwv044}{What is the contribution to the HIV epidemic of dynamic variation in sexual behavior of an individual over time? How does this compare to the role of heterogeneity between individuals?}
  \item \link{http://dx.doi.org/10.1371/journal.pcbi.1000898}{What explains the interannual variability of malaria?}
  \item \link{http://dx.doi.org/10.1098/rspb.2015.0347}{What will happen next in an Ebola outbreak?}
  \item \link{http://doi.org/10.1016/j.advwatres.2016.11.012}{Can hydrology explain the seasonality of cholera?}
  \item \link{http://doi.org/10.1073/pnas.1323688111}{What is the contribution of adults to polio transmission?}
  \end{enumerate}
\end{frame}
...
\begin{frame}{Schematic of the structure of a POMP}
  ...
  \end{itemize}
  \begin{center}
    \includegraphics[height=5cm]{../graphics/pomp_schematic1.png}
  \end{center}
\end{frame}
...
\section{The \package{pomp} package}

\begin{frame}{The \package{pomp} package for POMP models}
  \begin{itemize}
  \item \package{pomp} is an  \Rlanguage package for data analysis using partially observed Markov process (POMP) models \citep{King2016}.
  \item Note the distinction:
    lower case \package{pomp} is a software package;
    upper case POMP is a class of models.
  \item \package{pomp} builds methodology for POMP models in terms of arbitrary user-specified POMP models.
  \item \package{pomp} provides tools, documentation, and examples to help users specify POMP models.
  \item \package{pomp} provides a platform for modification and sharing of models, data-analysis workflows, and methodological development.
  \end{itemize}
\end{frame}

\begin{frame}{Structure of the \package{pomp} package}
  It is useful to divide the \package{pomp} package functionality into different levels:
  \begin{itemize}
...
\begin{frame}{Basic model components}
  ...
  \begin{itemize}
  \item \code{rinit}: simulator for the initial-state distribution, i.e., the distribution of the latent state at time $t_0$.
...
\begin{frame}{POMP inference algorithms}
  ...
  among the estimation algorithms just listed, four are methods that construct stateful objective functions that can be optimized using general-purpose numerical optimization algorithms such as \code{optim}, \code{subplex}, or the optimizers in the \package{nloptr} package.
\end{frame}
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \link{https://kingaa.github.io/pomp/}{\package{pomp} homepage}
