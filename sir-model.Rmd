...
read.csv(url,comment.char="#",colClasses=c(date="Date")) %>%
  ...
  subset(select=-c(status,date)) -> bsflu
bsflu %>% ggplot(aes(x=day,y=cases))+geom_line()+geom_point()
```
...
```{r add-meas-model}
bsflu %<>% pomp(rmeasure=Csnippet(rmeas),dmeasure=Csnippet(dmeas),
              statenames="H",paramnames="rho")
```

```{r}
simulate(bsflu,params=c(Beta=2,gamma=0.1,rho=0.9),nsim=20,as=TRUE,include=TRUE) %>%
...
bsflu %<>% pomp(toEstimationScale=Csnippet(toEst),
              fromEstimationScale=Csnippet(fromEst),
              paramnames=c("Beta","gamma","rho"))
```

## Sequential Monte Carlo in `pomp` (3B)
...
```{r bsflu-like-slice,cache=T,results='hide'}
require(pomp)
require(foreach)
require(doMC)
