...
Here, we see that the most successful searches have always led to models with reporting rate around 1%. 
This impression can be reinforced by looking at results from the global searches:

```{r global_rho}
plot(loglik~rho,data=subset(params,loglik>max(loglik)-10),log="x")
```

We see that reporting rates close to 1% seem to provide a small but clear (several units of log likelihood) advantage in explaining the data.
...
Note that $\rho$ is not estimated.
...
For the data, there were `r with(subset(mean_zeros,data),mean)` months with no reported cases, in reasonable accordance with the mean of `r with(subset(mean_zeros,!data),round(mean,1))` for simulations from the fitted model. 
...
Months with fewer than 100 infections averaged `r round(mean(fadeouts$fadeout100),1)` per simulation, which in the context of a reporting rate of `r signif(coef(polio,"rho"),3)` can explain the absences of case reports. 
For this model, the mean monthly infections due to importations (more specifically, due to the term $\psi$, whatever its biological interpretation) is `r round(imports,1)`. 
