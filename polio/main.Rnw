...
\item 
  Here, we set up the code to run on either a local machine or a slurm cluster  
\ei
...
\item In  \Sexpr{myround(.system.time[3],1)} seconds, we obtain a log likelihood estimate of \Sexpr{myround(L1[1],2)} with a Monte Carlo standard error of \Sexpr{myround(L1[2],2)}.
