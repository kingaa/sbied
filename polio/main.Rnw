...
@
  ...
  \bi
\item Our task, like most statistical computing, is \myemph{embarrassingly parallel} so we can use a simple parallel for loop via \code{foreach()}
  \ei
<<cache-cores>>=
...
@
\bi
\item We set up a cache directory and record the number of cores used for the cached results.
\ei
