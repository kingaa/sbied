...
\begin{frame}[fragile]
  ...
  Months with no asyptomatic infections for the simulations were rare, on average \Sexpr{round(fadeout1_sim,1)} months per simulation.
...
\item The aim of mechanistic modeling here is not to beat non-mechanstic models, but it is comforting that we're competitive with them.
