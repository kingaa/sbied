...
\item In  \Sexpr{round(.system.time[3],1)} seconds, we obtain a log likelihood estimate of \Sexpr{round(L1[1],2)} with a Monte Carlo standard error of \Sexpr{round(L1[2],2)}.
...
\item  either way, the computation will not be re-run unless you edit the code, change something on which the computation depends, or manually delete the archive file (\code{results/pf1.rda}).
...
\item This investigation took \Sexpr{round(.system.time[3]/60,1)}~min.
