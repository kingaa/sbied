...
\begin{frame}[fragile]
  ...
  \item Named covariate time series entered via the \code{covar} argument to \code{pomp} are automatically defined within C snippets used for the \code{rprocess}, \code{dprocess}, \code{rmeasure}, \code{dmeasure} and \code{rinit} arguments.
...
\begin{frame}[fragile]
  \frametitle{Setting up parallelization}

  We will use \pkg{doFuture} to parallelize our computations.
...
\begin{frame}[fragile]
  ...
  \item It took $\Sexpr{round(.system.time["elapsed"]/60,1)}$ minutes to run on $\Sexpr{cores}$ cores.
  \end{itemize}
...
\begin{frame}[fragile,allowframebreaks]
  ...
  <<mif_diagnostics1,echo=TRUE,eval=TRUE,purl=FALSE>>=
  ...
  <<mif_diagnostics2,echo=TRUE,eval=TRUE,purl=FALSE>>=
  best |> traces() |> melt() |>
...
\begin{frame}[fragile,allowframebreaks]
  ...
  <<likelihood_convergence,out.width="0.8\\linewidth",purl=FALSE>>=
