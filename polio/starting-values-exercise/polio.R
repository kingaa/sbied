#' ---
#' title: "Modification to test alternative starting value specification. Case study: polio in Wisconsin"
...
stew(file="local_search.rda",{
  ...
  t1 <- system.time({
    ...
    foreach(
      ...
      polio %>%
        mif2(
          Np=if(DEBUG) 50 else 1000,
          Nmif=if(DEBUG) 5 else 50,
      ...
      replicate(
        10, 
        mf %>% pfilter(Np=if(DEBUG) 50 else 5000) %>% logLik()
      ) %>%
...
polio_box <- rbind(
  ...
  rho=c(0.001,0.1),
  tau=c(0.001,0.1),
  sigma_dem=c(0.005,0.5),
  sigma_env=c(0.01,1),
  SO_0=c(0.01,0.99),
  IO_0=c(0.0001,0.01),
  cbind(fixed_params,fixed_params)
)
trans_box <- partrans(polio,polio_box,dir="toEst")
...
stew(file="global_search.rda",{
  ...
  t2 <- system.time({
    foreach(
      i=1:ifelse(DEBUG,20,400),
      .packages='pomp',.combine=rbind
    ) %dopar% {      
      trans_guess <- runif(nrow(trans_box),trans_box[,1],trans_box[,2])
      names(trans_guess) <- rownames(trans_box)
      guess <- partrans(polio,trans_guess,dir="fromEst")
      polio %>% mif2(
        params=guess,
        Np=if(DEBUG) 200 else 2000,
        Nmif=if(DEBUG) 5 else 300,
        cooling.type="geometric",
        cooling.fraction.50=0.5,
        rw.sd=rw.sd(
          b1=0.02, b2=0.02, b3=0.02, b4=0.02, b5=0.02, b6=0.02,
...
bake(file="profile_rho.rds",{
  ...
  foreach(
    ...
    polio %>%
      mif2(
        params=unlist(start),
        Np=if(DEBUG) 20 else 2000,
        Nmif=if(DEBUG) 3 else 300,
        cooling.type="geometric",
        cooling.fraction.50=0.5,
        rw.sd=rw.sd(
          b1=0.02, b2=0.02, b3=0.02, b4=0.02, b5=0.02, b6=0.02,
      ...
      mif2(
        Np=if(DEBUG) 50 else 5000,
        Nmif=if(DEBUG) 5 else 100,
