...
```{r model-construct,echo=FALSE,purl=TRUE}
library(tidyverse)
library(pomp)
...
```{r like-slice,cache=TRUE,results='hide'}
sliceDesign(
  center=coef(measSIR),
...
```{r like-slice-plot,cache=FALSE,echo=FALSE}
library(tidyverse)
...
```
```{r pfilter-grid1-plot,echo=F,purl=T}
p %>% 
