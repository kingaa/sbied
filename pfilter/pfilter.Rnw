...
\begin{frame}[fragile,allowframebreaks]{Particle filtering in \package{pomp}}
  ...
  The \Rlanguage code to do this is available \link{http://raw.githubusercontent.com/kingaa/sbied/master/pfilter/pfilter.R}{for download}.
...
\begin{frame}{Profile likelihood estimation}
  ...
  We will explain this method below and put it into practice in the \link{../mif/slides.pdf}{next lesson}.
\end{frame}
...
\begin{frame}[allowframebreaks]{Akaike's information criterion (AIC)}
  ...
  \item
    ...
    One can penalize complexity by incorporating a more severe penalty that the $2D$ term above, such as via \link{https://en.wikipedia.org/wiki/Bayesian_information_criterion}{BIC}.
  \item
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \link{https://raw.githubusercontent.com/kingaa/sbied/master/pfilter/pfilter.R}{\Rlanguage codes for this lesson}
\end{frame}
