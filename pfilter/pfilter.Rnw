...
\title{Lesson 2:\\Likelihood-based inference for POMP models}
...
\begin{frame}[fragile,allowframebreaks]{The likelihood for a POMP model}
  ...
  \begin{itemize}
  \item The latent process $X(t)$ may be defined at all times, but we are particulary interested in its value at observation times. Therefore, we write 
    $$X_n=X(t_n).$$ 
...
\begin{frame}[allowframebreaks]{Monte Carlo likelihood by direct simulation}
  ...
  \item Although \textbf{this section is a demonstration of what not to do}, it serves as an introduction to the general approach of \href{monteCarlo.html\#the-fundamental-theorem-of-monte-carlo-integration}{\textcolor{blue}{Monte Carlo integration}}.
  \item First, let's rewrite the likelihood integral using an equivalent factorization. As an exercise, you could check how the equivalence of Eqns.~\ref{eq:L1} and \ref{eq:L2} follows algebraically from the Markov property and the definition of conditional density.
  \end{itemize}
  ...
  \item Now, using a \href{https://en.wikipedia.org/wiki/Law_of_large_numbers}{\textcolor{blue}{law of large numbers}}, we can approximate an expectation by the average of a Monte Carlo sample. Thus,
    \begin{equation*}
  ...
  \item We can see this happening in practice for the measles outbreak data: \href{directSimulation.html}{\textcolor{blue}{supplementary material}}.
  \end{itemize}
...
\begin{frame}[fragile,allowframebreaks]{Particle filtering in \package{pomp}}
  ...
  The \Rlanguage code to do this is available \href{http://raw.githubusercontent.com/kingaa/sbied/master/pfilter/pfilter.R}{\textcolor{blue}{for download}}.
...
\begin{frame}[allowframebreaks]{Fisher information}
  ...
  \item
    A review of standard errors via Fisher information is provided as a \href{fisherSE.html}{supplement}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Profile likelihood estimation}
  ...
  We will explain this method below and put it into practice in the \href{../mif/slides.pdf}{\textcolor{blue}{next lesson}}.
\end{frame}
...
\begin{frame}{The Bootstrap}
  ...
  \item
    Further discussion of bootstrap methods for POMP models is provided as a \href{bootstrap.html}{supplement}.
  \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Confidence intervals via the profile likelihood}
  ...
  \item
    ...
    \end{equation*}
  \item
    This is known as a profile likelihood confidence interval. The cutoff $1.92$ is derived using \href{https://en.wikipedia.org/wiki/Likelihood-ratio_test\#Distribution:_Wilks.27s_theorem}{Wilks' theorem}, which we will discuss in more detail when we develop likelihood ratio tests.
  \item
...
\section{The geometry of inference}
...
\begin{frame}[fragile,allowframebreaks]{Exploring the likelihood surface: slices}
  \begin{itemize}
  \item
    To get an idea of what the likelihood surface looks like in the neighborhood of a point in parameter space, we can construct some likelihood \emph{slices}. We'll make slices in the $\beta$ and $\mu_{IR}$ directions. Both slices will pass through the central point.
  \item
  ...
  \begin{itemize}
  \item
    Note that we've used the \package{foreach} package with the \package{doParallel} backend to parallelize these computations.
  \item
    To ensure that we have high-quality random numbers in each parallel \Rlanguage session, we use a parallel random number generator provided by the \package{doRNG} package and initialized by the \code{registerDoRNG} call.
  \end{itemize}
...
\begin{frame}{Basic Exercise: Cost of a particle-filter calculation}
  \begin{itemize}
  \item
  ...
  \href{http://raw.githubusercontent.com/kingaa/sbied/master/pfilter/expense.R}{\textcolor{blue}{Worked solution to the Exercise}}
\end{frame}

\begin{frame}{Basic Exercise: Log likelihood estimation}
  ...
  \href{./loglikest.html}{\textcolor{blue}{Worked solution to the Exercise}}
\end{frame}

\begin{frame}{Optional Exercise: one-dimensional likelihood slice}
  Compute several likelihood slices in the $\eta$ direction.
\end{frame}

\begin{frame}{Optional Exercise: two-dimensional likelihood slice}
...
\begin{frame}[allowframebreaks]{Maximizing the particle filter likelihood}
  ...
  \item
    Further investigation into this approach is available as a \href{pf-in-Nelder-Mead.html}{supplement}.
  \item
...
\begin{frame}[allowframebreaks]{Wilks' theorem and profile likelihood}
  ...
  \item
    This is an example of \href{http://www.stat.nus.edu.sg/~wloh/lecture17.pdf}{\textcolor{blue}{a general duality between confidence intervals and hypothesis tests}}.
  \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Akaike's information criterion (AIC)}
  ...
  \item
    ...
    It can be a mistake to interpret AIC by making a claim that the favored model has been shown to provides a superior explanation of the data.
  ...
  \item
    ...
    One can penalize complexity by incorporating a more severe penalty that the $2D$ term above, such as via \href{https://en.wikipedia.org/wiki/Bayesian_information_criterion}{\textcolor{blue}{BIC}}.
  \item
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \item
    This lesson is prepared for the \href{https://kingaa.github.io/sbied/}{Simulation-based Inference for Epidemiological Dynamics} module at the 2020 Summer Institute in Statistics and Modeling in Infectious Diseases, \href{https://www.biostat.washington.edu/suminst/sismid}{SISMID 2020}.

  \item
    The materials build on \href{../acknowledge.html}{previous versions of this course and related courses}.

  \item
    Produced with R version \Sexpr{getRversion()} and \package{pomp} version \Sexpr{packageVersion("pomp")}.

  \item
    Licensed under the \href{http://creativecommons.org/licenses/by-nc/4.0/}{\textcolor{blue}{Creative Commons Attribution-NonCommercial license}}.
    Please share and remix non-commercially, mentioning its origin.\\
    \includegraphics[height=24pt]{../graphics/cc-by-nc}
  \end{itemize}

\href{../index.html}{\textcolor{blue}{Back to course homepage}}\\
  \href{https://raw.githubusercontent.com/kingaa/sbied/master/pfilter/pfilter.R}{\textcolor{blue}{\Rlanguage code for this lesson}}\\
\end{frame}
