...
\begin{frame}[fragile,allowframebreaks=0.8]
  ...
  The first two lines above load the \pkg{foreach} and \pkg{doParallel} packages, the latter being a ``backend'' for the \pkg{foreach} package.
  The next line tells \pkg{foreach} that we will use the \pkg{doParallel} backend.
...
\begin{frame}[fragile]{Parallel random number generators (RNG)}
  To initialize a parallel RNG, we use the \pkg{doRNG} package.
...
\begin{frame}[fragile,allowframebreaks]{Particle filtering in \pkg{pomp}}
  ...
  Here, we repeat the construction of the SIR model we looked at earlier, using parameters obtained by looking at simulations.
...
\begin{frame}{\myexercise. Log likelihood estimation}
  ...
  \item
    Use \pkg{doParallel} to take advantage of multiple cores on your computer to improve your estimate.
  \end{itemize}
