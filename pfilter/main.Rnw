...
\begin{frame}[fragile,allowframebreaks]{Particle filtering in \package{pomp}}
  ...
  \Rlanguage code to build the model is available \link{http://raw.githubusercontent.com/kingaa/sbied/master/pfilter/model.R}{for download}.

  <<read_chunk,include=FALSE,purl=FALSE>>=
