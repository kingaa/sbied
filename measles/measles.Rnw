...
\item To demonstrate the use of covariates in **pomp**.
...
\begin{frame}[fragile]
\frametitle{Motivation: challenges in inference from disease dynamics}
\begin{itemize}
...
\item See @Rohani2010 for a review of some of the high points.
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{City case counts}
<<dataplot,echo=F,fig.height=12,purl=F>>=
demog %>%
  group_by(town) %>%
...
<<seir-diagram,eecho=FALSE,cache=FALSE,purl=FALSE,eval=FALSE>>=
library(DiagrammeR)
...
\begin{itemize}
    ...
    \item $\zeta(t) = \text{Gamma white noise with intensity}\,\sigma_{SE}$ [@He2010;@bhadra11]
...
\begin{frame}[fragile]
\frametitle{Implementation in **pomp**}
\begin{itemize}
\item We'll load the packages we'll need, and set the random seed, to allow reproducibility.
\item Note that we'll be making heavy use of the \myemph{tidyverse} methods.
\item Also, we'll be using \myemph{ggplot2} for plotting: see \href{https://kingaa.github.io/R_Tutorial/viz.html#a-more-systematic-approach-the-grammar-of-graphics}{this brief tutorial}.
\item Finally, we'll use the convenient **magrittr** syntax, which is explained \href{https://kingaa.github.io/R_Tutorial/munging.html#the-magrittr-syntax}{here}.
\end{itemize}
<<prelims2,cache=FALSE>>=
library(pomp)
library(tidyverse)
theme_set(theme_bw())
set.seed(594709947L)
...
\begin{frame}[fragile]
\frametitle{Data and covariate plots}
<<data-plot>>=
...
\item The birth-cohort effect: a specified fraction (`cohort`) of the cohort enter the susceptible pool all at once.
...
\begin{itemize}
\item In the above, $C$ represents the true incidence, i.e., the number of new infections occurring over an interval.
...
The full set is included in the \myemph{R} code accompanying this document, where they are read into a data frame called `mles`.
<<mles,include=FALSE>>=
read.csv(text="
...
We verify that we get the same likelihood as @He2010. \textcolor{red}{Change eval to T}
...
\begin{frame}[fragile]
\frametitle{Results from @He2010}
...
<<delay,eval=TRUE,echo=FALSE,results="hide",purl=FALSE,fig.cap="Profile likelihood for birth-cohort delay, showing 95\\% and 99\\% critical values of the log likelihood.">>=
...
\hspace{3cm} {\large \bf $R_0$}
\begin{itemize}
\item Recall that $R_0$ is the basic reproduction number: a measure of how communicable an infection is.
\item Existing estimates of $R_0$ (c. 15--20) come from two sources:
    \begin{itemize}
    \item serology surveys
    \item models fit to data using feature-based methods
    \end{itemize}
\end{itemize}
<<R0,echo=FALSE,results="hide",purl=FALSE>>=
...
<<est-table,echo=FALSE,purl=FALSE>>=
...
\begin{itemize}
\item Modify the @He2010 model to remove the cohort effect.
