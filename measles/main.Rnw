...
\begin{frame}[fragile]{Implementation in \pkg{pomp}}
  ...
  \item In particular, we use the convenient \pkg{magrittr} pipe syntax, which is explained \link{https://kingaa.github.io/R_Tutorial/munging.html\#the-magrittr-syntax}{elsewhere}.
  \end{itemize}
\end{frame}
...
pf_reps <- bake("pfilter-reps.rds",{
  ...
  foreach(i=1:20,.options.future=list(seed=50505),.combine=cbind) %dofuture% {
    cond_logLik(pfilter(m1,Np=2000,params=unlist(as.vector(mle[-(1:3)]))))
  }
})
@
...
plot(y=anomaly,x=time(m1),xlab="Date",ylab="Anomaly")
@
\begin{itemize}
...
par(mai=c(0.8,0.8,0.1,0.1))
plot(apply(pf_reps,1,var),x=time(m1),xlab="Date",ylab="Variance")
@
\begin{itemize}
