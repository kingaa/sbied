...
\begin{frame}[allowframebreaks]{Data and covariate plots}
  ...
  <<covarplot,out.width="1\\textwidth",echo=FALSE,purl=FALSE>>=
  library(cowplot)
  demogLondon %>%
    select(year,pop) %>%
...
\begin{frame}[fragile]
  \frametitle{Process model observations}
  \begin{itemize}
...
\begin{frame}[fragile]
  ...
  <<pfilter1,eval=T,cache=T,size="small">>=
...
\begin{frame}[fragile]
  \frametitle{Predicted vs observed critical community size}
...
\begin{frame}[fragile]
  \frametitle{$R_0$ estimates inconsistent with literature}
  \begin{itemize}
...
\begin{frame}[fragile]
  \frametitle{Extrademographic stochasticity}
...
\begin{frame}{License, acknowledgments, and links}
  ...
  \link{https://raw.githubusercontent.com/kingaa/sbied/master/measles/main.R}{\Rlanguage code for this lesson}
