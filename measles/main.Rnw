...
<<plot_colors,include=FALSE,purl=FALSE>>=
line.color <- "red"
plot.color <- "black"
@
...
<<prelims,echo=F,cache=F,purl=FALSE>>=
@
...
\begin{frame}[fragile,allowframebreaks=0.8]{Challenges in inference from disease dynamics}
  ...
  \end{itemize}
  <<load-data,echo=FALSE,purl=FALSE>>=
  @
...
<<rproc_listing,echo=FALSE,results="asis",purl=F>>=
cat(rproc@text)
@ 
\end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{State initializations}
  ...
  @
  \begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
    <<rinit_listing,echo=FALSE,results="asis",purl=F>>=
    cat(rinit@text)
    @ 
  \end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{The measurement model II}
  ...
  \begin{lstlisting}[basicstyle=\ttfamily\tiny]
    <<dmeas_listing,echo=FALSE,results="asis",purl=F>>=
    cat(dmeas@text)
    @ 
  \end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{Case simulations}
  ...
  @
  \begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
    <<rmeas_listing,echo=FALSE,results="asis",purl=F>>=
    cat(rmeas@text)
    @ 
  \end{lstlisting}
\end{frame}
...
\subsection{\cite{He2010}}

\begin{frame}[fragile]{Estimates from \cite{He2010}}
  ...
  We verify that we get the same likelihood as \cite{He2010}.

  <<pfilter1a,eval=F,size="small">>=
  @
  <<pfilter1_eval,eval=T,include=F,cache=F,purl=F>>=
  bake("pfilter1.rds",{
    <<pfilter1a>>
  }) -> pfs
  @ 
...
\begin{frame}[fragile]{Parameter transformations}
  ...
  <<transforms,size="scriptsize">>=
  @
\end{frame}

\section{Findings}

\begin{frame}[fragile]{Results from \cite{He2010}}
...
\begin{frame}[fragile]{Imported infections}
  ...
  <<imports,results="hide",echo=FALSE,purl=FALSE,out.width="4in">>=
...
\begin{frame}[fragile]{Birth delay}
  <<delay,eval=TRUE,echo=FALSE,results="hide",purl=FALSE,fig.height=5,out.width="0.7\\textwidth">>=
...
\begin{frame}[fragile]{Parameter estimates}
  <<est-table,echo=FALSE,size="tiny",purl=FALSE>>=
...
\begin{frame}[fragile]{Extrademographic stochasticity}
  ...
  <<env-noise,echo=FALSE,results="hide",purl=FALSE,out.width="4in">>=
