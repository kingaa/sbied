...
1. display a published case study using plug-and-play methods with non-trivial model
1. demonstrate the use of profile likelihood
...
```{r seir-diagram1,echo=FALSE,cache=FALSE,purl=FALSE}
require(DiagrammeR)
...
```{r esttable,echo=FALSE,purl=FALSE}
load("twentycities.rda")
...
knitr::kable(subset(mle,select=-c(S_0,E_0,I_0,R_0)),row.names=FALSE)
```
...
```{r sigmaSE-prof-design,eval=TRUE}
...
profileDesign(
  sigmaSE=exp(seq(from=log(0.02),to=log(0.2),length=10)),
  lower=theta.lo,upper=theta.hi,nprof=20
) -> pd
```
...
```{r sigmaSE-prof,eval=TRUE}
bake("sigmaSE-profile.rds",{
